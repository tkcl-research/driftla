...
class LightGCN(nn.Module):
    ...
    def __init__(
        ...
        self.ego_skip_alpha = float(ego_skip_alpha)
    ...
    def computer(self, adj_matrix):
        ...
        if self.ego_skip_alpha > 0.0:
            ego_all = embs[:, 0, :]
            a = min(max(float(self.ego_skip_alpha), 0.0), 1.0)
            light_out = (1.0 - a) * light_out + a * ego_all
